...
classdef helper
    ...
    methods
        ...
        function test(obj,testdata,max_iteration,max_sample_rate,endindex)
            ...
            for test_run = 1:size(testdata,3)
                % Test image data + Normalize
                refnorm = img_norm(testdata(:,:,test_run));
                ...
                for i = 1:max_iteration
                    ...
                    flip_mask = 1.-a;
                    % flip 1 to 0 // 0 to 1
                    top10page = my_av_dist(obj.data,correct10,obj.number_input,length(Crow),Crow,Ccol,16);
                    % Recover image with mask
                    Final_rec = testspec .* a;
                    % stored_mask(:,:,i)=a;
                    output = img_norm(rec(Final_rec));
                    ...
                    if (i == 6 || i == 12 || i == 18 || i == 24 || i == 30) && 0
                        figure
                        sgtitle(['Recovered @ sampling rate ',num2str(round(samplingrate)),' %'])
                        subplot(2,2,1)
                        imshow(outputav)
                        title('Average')
                        subplot(2,2,3)
                    ...
                    SNRarrayav(i,1) = SNRarrayav(i,1) + psnr(outputav,refnorm);
                    SNRarraytest(i,1) = SNRarraytest(i,1) + psnr(outputtest,refnorm);
                    SNRarrayzig(i,1) = SNRarrayzig(i,1) + psnr(outputzig,refnorm);
                    ssimarrayav(i,1) = ssimarrayav(i,1) + ssim(outputav,refnorm);
                    ssimtest(i,1) = ssimtest(i,1) + ssim(outputtest,refnorm);
                    ssimzig(i,1) = ssimzig(i,1) + ssim(outputzig,refnorm);
                    energy_av(i,1)= energy_av(i,1) + sum(sum(abs(needrecav)))/sum(sum(abs(testspec)));
                    energy_test(i,1)= energy_test(i,1) + sum(sum(abs(needrectest)))/sum(sum(abs(testspec)));
                    energy_zig(i,1)= energy_zig(i,1) + sum(sum(abs(needreczigzag)))/sum(sum(abs(testspec)));
                    
                    % set sampling rate
                    SNRarrayav(i,2) = SNRarrayav(i,2) + samplingrate;
                    SNRarraytest(i,2) = SNRarraytest(i,2)+ samplingrate;
                    SNRarrayzig(i,2) = SNRarrayzig(i,2)+samplingrate;
                    ssimarrayav(i,2) = ssimarrayav(i,2) + samplingrate;
                    ssimtest(i,2) = ssimtest(i,2)+samplingrate;
                    ssimzig(i,2) = ssimzig(i,2)+samplingrate;
                    energy_av(i,2)= energy_av(i,2)+samplingrate;
                    energy_test(i,2)= energy_test(i,2)+samplingrate;
                    energy_zig(i,2)= energy_zig(i,2)+samplingrate;
                    
                    % PSNR & SSIM
                    SNRarrayOP(i,1) = SNRarrayOP(i,1) + psnr(output,refnorm);
                    ssimOP(i,1)= ssimOP(i,1) + ssim(output,refnorm);
                    samplingrate = 100*sum(sum(a))/obj.N^2;
                    SNRarrayOP(i,2) = SNRarrayOP(i,2) + samplingrate;
                    ssimOP(i,2) = ssimOP(i,2) + samplingrate;
%                     energyOP(i,1)= energyOP(i,1) + ((sum(abs(Final_rec)))/sum(sum(abs(testspec))));
%                     energyOP(i,2)= energyOP(i,2) + samplingrate;
                end
                toc;
            end
            ...
            if obj.PRINTFLAG == 1
                ...
                NORM_av = size(testdata,3);
                % Code for PSNR plot
                figure
                plot(SNRarrayav(:,2)./NORM_av,SNRarrayav(:,1)./NORM_av,'LineWidth',1.5,'Color','b')
                grid on
                grid minor
                % hold on
                % plot (snrown(:,2),snrown(:,1),'r--');
                hold on
                plot (SNRarrayOP(:,2)./NORM_av,SNRarrayOP(:,1)./NORM_av,'LineWidth',1.5,'Color','m')
                hold on
                plot (SNRarrayzig(:,2)./NORM_av,SNRarrayzig(:,1)./NORM_av,'LineWidth',1.5,'Color','k')
                ...
                % Code for SSIM plot
                figure
                plot (ssimarrayav(:,2)./NORM_av,ssimarrayav(:,1)./NORM_av,'LineWidth',1.5,'Color','b');
                grid on
                grid minor
                hold on
                % plot(ssim_own(:,2),ssim_own(:,1),'r--')
                % hold on
                plot (ssimOP(:,2)./NORM_av,ssimOP(:,1)./NORM_av,'LineWidth',1.5,'Color','m')
                hold on
                plot (ssimzig(:,2)./NORM_av,ssimzig(:,1)./NORM_av,'LineWidth',1.5,'Color','k')
                lgd = legend('Average','Optimized','Zigzag','Location','northwest');
                lgd.FontSize = 30;
                lgd.FontName = 'Times New Roman';
                hold off
...
%                 figure
%                 plot(energy_av(:,2)./NORM_av,energy_av(:,1)./NORM_av,'LineWidth',1.5,'Color','b')
%                 grid on
%                 grid minor
%                 hold on
%                 % plot(energy_own(:,2),energy_own(:,1),'r--')
%                 plot(energy_test(:,2)./NORM_av,energy_test(:,1)./NORM_av,'LineWidth',1.5,'Color','m')
%                 plot(energy_zig(:,2)./NORM_av,energy_zig(:,1)./NORM_av,'LineWidth',1.5,'Color','k')
%                 hold off
%                 lgd = legend('Average','Optimized','Zigzag','Location','northwest');
%                 lgd.FontSize = 30;
%                 lgd.FontName = 'Times New Roman';
